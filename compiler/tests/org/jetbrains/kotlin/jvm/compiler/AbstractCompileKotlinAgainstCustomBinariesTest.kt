...
abstract class AbstractCompileKotlinAgainstCustomBinariesTest : AbstractKotlinCompilerIntegrationTest() {
    ...
    */
    ...
    fun testAgainstFir() {
        val library = compileLibrary("library", additionalOptions = listOf("-language-version", "2.1"))
        compileKotlin("source.kt", tmpdir, listOf(library))
    }

    fun testAgainstFirWithUnstableAbi() {
        val library2 = compileLibrary("library", additionalOptions = listOf("-language-version", "2.1", "-Xabi-stability=unstable"))
        compileKotlin("source.kt", tmpdir, listOf(library2))
    }
    ...
    fun testAgainstFirWithStableAbi() {
        val library = compileLibrary("library", additionalOptions = listOf("-language-version", "2.1", "-Xabi-stability=stable"))
        compileKotlin("source.kt", tmpdir, listOf(library))
    }
