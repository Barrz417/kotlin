...
val kdump.Type.hprofClassName: String get() =
  if (!isArray) {
...
val kdump.Type.hprofMappedClassName: String? get() =
  when (packageName) {
    "kotlin" ->
      when (relativeName) {
...
val kdump.Type.hprofDefaultClassName: String get() =
  hprofPackageName + hprofPackageSeparator + hprofRelativeName

val kdump.Type.hprofPackageName: String get() =
  packageName.replace(".", "/")

val kdump.Type.hprofPackageSeparator: String get() =
  if (packageName.isEmpty()) "" else "/"

val kdump.Type.hprofRelativeName: String get() =
  relativeName.replace(".", "$")

val kdump.Type.forceFields: List<Field> get() = fields ?: listOf() //throw IOException("No fields")

val RuntimeType.hprofTypes: List<Type> get() =
  when (this) {
